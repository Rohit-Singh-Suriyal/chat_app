...
export function DrawerWithNav(props) {
  ...
  React.useEffect(() => {
    if (allRoomIds?.length > 0) {
      console.log("allROom", allRoomIds);
  ...
  async function searchUsersByDisplayName(displayName) {
    const displayNameL = displayName.toLowerCase();
    try {
      const q = query(
        collection(db, "users"),
        where("queryName", ">=", displayNameL),
        where("queryName", "<=", displayNameL + "z")
      );

      const allUsersAfterSearch = [];
      const roomids = [];

      // Loop through the matched documents and extract user IDs
      onSnapshot(q, (qSnapshot) => {
        setAllUsers(
          qSnapshot.docs.map((doc) => {
            if (doc.data().uid != user.uid) {
              let roomid = [user.uid, doc.data().uid].sort();
              roomid = roomid[0] + roomid[1];
      ...
      // setAllUsers(allUsersAfterSearch);
      return allUsersAfterSearch;
    } catch (error) {
  ...
  useEffect(() => {
    console.log("searcTrm", searchTerm);
    if (searchTerm !== "") {
      setAllRoomIds([]);
      searchUsersByDisplayName(searchTerm)
        .then((allUsersAfterSearch) => {
          // console.log("allUsersafterSearch", allUsersAfterSearch);
          // setAllUsers(allUsersAfterSearch);
        })
        .catch((error) => {
